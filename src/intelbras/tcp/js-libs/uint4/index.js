...
module.exports = exports = Uint4;
// compatibility
exports.Uint4 = Uint4;
function Uint4() {
  if (!(this instanceof Uint4)) return new Uint4();
  function readUInt4LE(buffer, cursor) {
    if (cursor % 1) return buffer.readUInt8(Math.floor(cursor)) >> 4;
    else return buffer.readUInt8(cursor) & 15;
  }
  function writeUInt4LE(buffer, value, cursor) {
    if (value >= 16) throw new Error("value is out of bounds");
    var byteLoc = Math.floor(cursor);
    if (cursor % 1)
      buffer.writeUInt8(
        (value << 4) | readUInt4LE(buffer, Math.floor(cursor)),
        byteLoc
      );
    else
