...
function intField(p, offset, length, le, signed) {
  ...
  var nativeSuff =
      (signed ? "" : "U") + "Int" + length * 8 + (le ? "LE" : "BE"),
    readMethod = buffer_1.Buffer.prototype["read" + nativeSuff],
    writeMethod = buffer_1.Buffer.prototype["write" + nativeSuff];
  if (!readMethod) {
    this.get = function () {
      var bor = le ? lec : bec;
...
function floatField(p, offset, le) {
  ...
  this.set = function (val) {
    return le
      ? p.buf.writeFloatLE(val, offset)
      : p.buf.writeFloatBE(val, offset);
  };
}
function doubleField(p, offset, le) {
  ...
  this.set = function (val) {
    return le
      ? p.buf.writeDoubleLE(val, offset)
      : p.buf.writeDoubleBE(val, offset);
  };
}
...
function Struct() {
  ...
  this.allocate = function () {
    applyClosures(priv);
    if (buffer_1.Buffer.alloc) {
      priv.buf = buffer_1.Buffer.alloc(priv.len);
    } else {
      priv.buf = new buffer_1.Buffer(priv.len);
      priv.buf.fill(0);
    }
    allocateFields();
    priv.allocated = true;
  ...
  this.set = function (key, val) {
    ...
    } else if (buffer_1.Buffer.isBuffer(key)) {
      this._setBuff(key);
    } else {
      for (var k in key) {
        this.set(k, key[k]);
  ...
  function getFields() {
    ...
    Object.keys(priv.fields).forEach(function (key) {
      var setFunc, getFunc;
      if (
        priv.fields[key] instanceof structField ||
