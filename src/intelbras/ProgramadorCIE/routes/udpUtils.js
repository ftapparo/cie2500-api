...
var Cypher = {
  ...
  crypt: function (data) {
    ...
    var iv = Buffer.concat([crypto.randomBytes(8), new Buffer(8).fill(0)]);
    ...
    //iv = Buffer.concat([IvTest, new Buffer(8).fill(0)])
    //data = DataTest
    //seqnum = SeqNumTest

    //--------------------------------
    // Prepare seq. number
    var snbuf = new Buffer(2);
  ...
  decrypt(encdata) {
    ...
    //--------------------------------
    // Extract HMAC
    var hmac = new Buffer(encdata.slice(enclen - 8, enclen));
